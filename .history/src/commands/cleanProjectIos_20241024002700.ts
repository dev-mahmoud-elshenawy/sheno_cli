...
async function ensureFlutterArtifactsExist() {
    ...
    // Check for the existence of Flutter.xcframework
    if (!fs.existsSync(flutterXcframeworkPath)) {
      console.log(chalk.red("Flutter.xcframework not found. Running 'flutter precache --ios'..."));
  
      try {
        await execInIos("fvm flutter precache --ios"); // Ensuring using FVM if available
        console.log(chalk.green("Flutter.xcframework has been downloaded successfully."));
      } catch (error) {
        console.error(chalk.red(`Failed to run precache: ${error instanceof Error ? error.message : error}`));
      }
    } else {
      console.log(chalk.green("Flutter.xcframework exists. No need to run 'flutter precache --ios'."));
    }
  }
