...
async function ensureFlutterArtifactsExist() {
  ...
  if (!fs.existsSync(flutterXcframeworkPath)) {
    ...
    try {
      console.log(chalk.green("By FVM Downloading Flutter.xcframework..."));
      await execCommand("fvm flutter precache --ios");
      console.log(
        chalk.yellow("Flutter.xcframework has been downloaded successfully.")
      );
    } catch (error) {
      console.error(
        chalk.red(
...
export async function cleanIosProject(cleanCache: boolean) {
  try {
    ...
    console.log(chalk.("Running clean for iOS project..."));
    ...
    console.log(chalk.green("Deintegrating pods..."));
