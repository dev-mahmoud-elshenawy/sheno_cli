...
function getFlutterSdkPath(): string {
    const fvmDir = path.join(process.cwd(), ".fvm", "flutter_sdk"); // Adjust the path based on your FVM setup
  
    // Check if the FVM directory exists
    if (fs.existsSync(fvmDir)) {
      console.log(chalk.green("Using FVM Flutter SDK..."));
      return fvmDir;
    }
  
    // Fallback to global Flutter installation
    const globalFlutterPath = execSync("which flutter", { encoding: "utf8" }).trim();
  
    if (!fs.existsSync(globalFlutterPath)) {
      throw new Error("Flutter SDK not found. Please ensure Flutter is installed.");
    }
  
    return path.dirname(globalFlutterPath);
  }
function getFlutterXcframeworkPath(): string {
    const flutterSdkPath = getFlutterSdkPath();
...
async function ensureFlutterArtifactsExist() {
  const flutterXcframeworkPath = getFlutterXcframeworkPath();
