...
async function cleanWithFvm() {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        console.error(chalk.red(`Error during FVM clean: ${error}`)); // Handle unexpected error formats
      }
    }
  }
  ...
  async function cleanWithoutFvm() {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        console.error(chalk.red(`Error during non-FVM clean: ${error}`)); // Handle unexpected error formats
      }
    }
}